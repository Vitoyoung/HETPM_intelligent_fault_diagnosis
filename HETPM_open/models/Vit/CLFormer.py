...
class Vit_CLFormer_features_1d(nn.Module):      # 适用1024大小
    def __init__(self, ):       #
        super(Vit_CLFormer_features_1d, self).__init__()
        self.Vit_CLFormer_features = CLFormer()
        self.__in_features = 256

    def forward(self, x):
        x = self.Vit_CLFormer_features(x)
        return x
