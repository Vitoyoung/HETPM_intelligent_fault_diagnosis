...
cfgs = {"S": [[1, 2, 5, 16, 2], [1, 2, 3, 32, 2], [1, 1, 3, 256, 4]],
        "M": [],
        "B": []}
...
def convoformer_v1_small(in_channel = 1):
...
class Vit_ConvForNSE_features_1d(nn.Module):
    def __init__(self, ):       #
        super(Vit_ConvForNSE_features_1d, self).__init__()
        self.Vit_ConvForNSE_features = convoformer_v1_small()
        self.__in_features = 256

    def forward(self, x):
        x = self.Vit_ConvForNSE_features(x)
        return x
