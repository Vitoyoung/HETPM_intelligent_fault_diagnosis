...
def mcswint():
    model = MCSwin_T(in_c=1,
                     kernel_sizes=[15, 9, 5, 3],
                     strides=[2, 1, 1, 1],
                     out_channels=[64, 128, 128, 192],
                     dim=256, depth=6, num_heads=8, window_size=16)
    return model




class Vit_Swin_features_1d(nn.Module):
    def __init__(self, ):       #
        super(Vit_Swin_features_1d, self).__init__()
        self.Vit_Swin_features = mcswint()
        self.__in_features = 256

    def forward(self, x):
        x = self.Vit_Swin_features(x)
        return x
